    ...
    return (
        <nav className="w-full h-16 md:h-20 flex bg-pri-black">
            ...
            <div className="w-1/5 h-full flex justify-end items-center pr-6 relative" onClick={() => setProfileOptn(!profileOptn)}>
                ...
                <button onClick={() => signOut(auth)} className={`${profileOptn ? "" : "opacity-0"} transition-opacity duration-150 ease-in-out absolute top-full mt-2 right-2 h-full w-40 bg-pri-black z-50 rounded-lg font-semibold`}>
                    Signout
                </button>
            </div>
        </nav>
